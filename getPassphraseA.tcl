...
# ./getPassphrase.tcl
# OR
# Start tclsh and source ./getPassphrase.tcl, then call 'puts [genRandList $max $min $readAll]'
# tclsh
# source ./getPassphrase.tcl
# puts [genRandList $max]
# puts [genRandList $max $min $readAll]
# for {set i 0} {$i < 100} {incr i} {puts [genRandList $max $min $readAll]}
# Description END
...
proc genRandList {maxValue {minValue 0} {amtBytes 800}} {
  ...
  foreach {num} $randList {
    append randStr [lindex $::words [expr {$num % ($maxValue - $minValue + 1) + $minValue}]] " "
  }
